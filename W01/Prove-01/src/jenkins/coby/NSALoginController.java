package jenkins.coby;/*	Note that we're not using wildcards in our import statements like:
...
/**
* The jenkins.coby.NSALoginController class handles password hashing and verification
	...
	*/
	public static void hashUserPassword(User user) throws Exception {
		...
		if(!user.getPassword().matches(REGEX)) {
			WeakPasswordException problem = new WeakPasswordException("Weak Password");
			throw problem;
		}
